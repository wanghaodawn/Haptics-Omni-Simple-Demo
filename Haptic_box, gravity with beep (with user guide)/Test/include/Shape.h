...
class QHAPI Shape
{
		...
		friend class QHRenderer;//渲染器
		friend class TriMesh;//三角形网点格线图
		friend class Cursor;//光标，指针
		friend class DeviceSpace;//设备空间
		
		friend class Box;//盒子
		friend class Cone;//圆锥
		friend class Cylinder;//圆柱
		friend class Line;//线
		friend class Plane;//平面
		friend class Sphere;//球
		friend class Text;//文字
		friend class TriMesh;//三角形网点格线图
		...
		///硬度 Haptic m_Stiffness of the shape
		HLdouble m_Stiffness;
		///定义硬度在那一边 Which side the stiffness is applied
		HLenum m_StiffnessSide;
		
		///阻尼的大小 Haptic m_Damping present in the shape
		HLdouble m_Damping;
		///定义阻尼在哪一边 Which side the damping is applied
		HLenum m_DampingSide;

		///物体是否在旋转 Is the model Spinning?
		bool m_RotationFlag;
	    ///垂直旋转 spin velocity (normalised)
		HLdouble m_RotationVelocity;
		///旋转的轴 spin Axis
		hduVector3Dd m_RotationAxis;

		///物体是否有某个轨道 Is the model orbiting?
		bool m_RevolutionFlag;
		///轨道的速度 Orbital velocity (normalised)
		HLdouble m_RevolutionVelocity;
		///轨道的轴 Axis of orbit
		hduVector3Dd m_RevolutionAxis;
		///轨道中心 centre of the orbit
		hduVector3Dd m_RevolutionPoint;

		///物体是否被拖着 Is the model being dragged?
		bool m_DraggingFlag;
